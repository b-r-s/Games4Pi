...
foreach ($cssFile in $cssFiles) {
    ...
    # Improved regex: only match class selectors at the start of a line
    $matches = [regex]::Matches($content, '^[\s]*\.(?<class>[a-zA-Z0-9_-]+)', [System.Text.RegularExpressions.RegexOptions]::Multiline)
    $classes = $matches | ForEach-Object { $_.Groups['class'].Value } | Select-Object -Unique
